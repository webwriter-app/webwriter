...
export class TransformationFeature extends EditorFeature {
  ...
  moveZ(el: HTMLElement, forward=true, toFrontOrBack=false, cycle=false) {
    ...
    const j = cycle? Math.max(0, i + d) % n: Math.min(Math.max(0, i + d), n)
  ...
  updateInfo(deg?: number) {
    ...
    if(target) {
      ...
      this.#containingBlock = findContainingBlock(target, elStyle.position as "static" | "relative" | "sticky" | "absolute" | "fixed")
      if(!(this.#containingBlock instanceof Window)) {
  ...
  clearTransform() {
    document.body.querySelectorAll(".◆transform-target")
      .forEach(el => el.classList.remove("◆transform-target"))
    document.body.querySelectorAll(".◆transform-containing-block")
      .forEach(el => el.classList.remove("◆transform-containing-block"))
    document.body.querySelectorAll(".◆transform-scrolling-ancestor")
      .forEach(el => el.classList.remove("◆transform-scrolling-ancestor"))
    document.body.querySelectorAll(".◆transform-stacking-container")
