...
export class SaveForm extends LitElement {
  ...
  Tree() {
    return html`<sl-card class="tree">
      ...
      ${this.mode === "save" && !this.client.pickSave
        ? html`
            ${this.Filename()}
            <sl-icon-button
              slot="footer"
      ...
      ${SaveForm.formats.map(
        (format) => html`
          ...
          >
            ...
            <sl-icon
              slot="suffix"
              name=${SaveForm.formatIcons[format]}
            ></sl-icon>
          </sl-option>
        `
      )}
  ...
  handleConfirm = async () => {
    if (
      ...
      (await confirm(
        msg(
          `${
            this.url.searchParams.get("filename") ?? "This file"
  ...
  render() {
    return html`<form>
      ...
      ${this.client.searchDocuments && !this.client.pickSave
        ? this.Tree()
        : null}
      <footer>
        <sl-button
