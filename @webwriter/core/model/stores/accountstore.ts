...
import { FileClient, NpmClient, PocketbaseClient } from "../clients"
import { Environment } from "../environment"
import marshal from "../marshal"
import { FileAccount, NpmAccount, PocketbaseAccount } from "../schemas/accounts"

type AccountsMap = AccountStore["accounts"]
export type AccountTypeId = keyof AccountStore["accounts"]
export type Account<T extends AccountTypeId = AccountTypeId> = AccountsMap[T][string]
export type Client<T extends AccountTypeId = AccountTypeId> = InstanceType<(typeof AccountStore)["clients"][T]>
...
export class AccountStore {
  ...
  static accountTypeIdMap = new Map(Object.entries(this.accountType).map(([k, v]) => [v, k]))
  ...
  constructor(readonly Environment: Environment, accounts?: AccountStore["accounts"]) {
  ...
  get clients() {
    return Object.fromEntries(Object.keys(this.accounts).map(key => [
      key,
      Object.fromEntries(Object.keys((this.accounts as any)[key]).map(name => [
        name,
        this.getClient(key as any, name)
      ])),
    ])) as {file: {[k: string]: FileClient}} & Partial<{npm: Record<string, NpmClient>, pocketbase: Record<string, PocketbaseClient>}>
  }

  get clientTriples() {
    return Object.entries(this.clients)
      .flatMap(([type, clientsOfType]) => Object.entries(clientsOfType).map(
        ([name, client]) => [type, name, client])
      ) as unknown as [keyof typeof AccountStore["accountType"], string, Client][]
  }
  ...
  getClient<T extends AccountTypeId>(type: T, name: string) {
    ...
    const client = account
      ? new AccountStore.clients[type](account, this.Environment, async (newAccount) => this.updateAccount(newAccount)) as Client<T>
      : undefined
    return client
  }
  ...
  clientNameFromURL(url: URL) {
    const triple = this.clientTriples
    .find(([type, id, client]) => client === this.clientFromURL(url)) ?? []
