...
describe("DOMEditor stylesheets", () => {
  ...
  it("keeps the empty editing surface interactive and its caret in the shadow appendix", () => {
    ...
    expect(editorStyleString).toMatch(/\.◆element-selected\s*\{[\s\S]*?outline:\s*2px dotted var\(--sl-color-primary-400\);[\s\S]*?outline-offset:\s*2px;/)
    expect(editorStyleString).not.toMatch(/\.◆element-selected:hover\s*\{/)
    expect(editorStyleString).toMatch(/\.◆element-capture-selected,\s*\.◆element-capture-selected:hover\s*\{[\s\S]*?outline:\s*2px solid var\(--sl-color-primary-400\);[\s\S]*?outline-offset:\s*2px;/)
